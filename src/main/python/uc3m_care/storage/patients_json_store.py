    ...
    class __PatientsJsonStore(JsonStore):
        ...
        def add_item(self, item):
            ...
            if not isinstance(item,VaccinePatientRegister):
                raise VaccineManagementException(PatientsJsonStore.__PatientsJsonStore.INVALID_PATIENT_OBJECT)
                ...
                if (patient_recorded["_VaccinePatientRegister__registration_type"]
                    == item.vaccine_type) \
                        ...
                        (patient_recorded["_VaccinePatientRegister__full_name"]
                         == item.full_name):
                    raise VaccineManagementException(PatientsJsonStore.__PatientsJsonStore.PATIENT_ID_REGISTERED)
